...
def crawl_currency():
    ...
    req = Request("https://coinmarketcap.com/", headers={"User-Agent": 'Mozilla/5.0'}) # change to brave later
...
def update_currency():
    print('Updating data ..')
    req = Request("https://coinmarketcap.com/", headers={'User-Agent': 'Mozilla/5.0'})
    html = urlopen(req).read()
    bs = BeautifulSoup(html, 'html.parser')
    ...
    for row in rows:
        ...
        span_element = row.find('span', class_="profile__name")
        if span_element:
            values = row.find_all('div', class_="valuta")
            price = values[0].get_text().strip().replace('\n', '')
            market_cap = values[1].get_text().strip().replace('\n', '')
            ...
            print("Element not found, skipping this row") 
